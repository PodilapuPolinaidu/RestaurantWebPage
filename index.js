...
function renderAllCards(data) {
  data.map((item) => {
    document.getElementById(
      "box"
    ).innerHTML += `<div class="card card-1" style="width: 22rem;">
  ...
  <div class="">
    <div class="d-flex justify-content-between m-2">
      ...
      <select onchange="showMealType(event)" class='bg-primary text-white rounded border-0'>
        <option>Select Meal Type</option>
      ${item.mealType.map((dish) => {
        return `<option value=${dish}>${dish}</option>`;
      })}</select>
      </div>
    <p class="card-text">${
      item.instructions[0].length > 20
        ? item.instructions[0].slice(0, 40)
        : item.instructions[0]
    }</p>
    <div class='d-flex justify-content-between mb-2'>
      <button class="btn btn-${
        item.difficulty == "Easy" ? "success" : "danger"
      }">${item.difficulty}</button>
      <p>Rating ${item.rating} <i class="fa-solid fa-star" class=""></i></p>
    </div>
...
// Filter the cards based on the  meal type
function showMealType(e) {
  e.preventDefault();
  let newData;
  let inputValue = document.getElementById("input_search").value;
  console.log(inputValue.toLowerCase());
  if (inputValue == "") {
    alert("Enter meal type");
  } else {
    newData = allData.filter((dish) => {
      if (dish.mealType.includes(inputValue)) return dish;
    });
  }
...
// renderAllCards(allData);

// Live search functionality
let requiredData;
function letterSearch() {
  document.getElementById("input_search").addEventListener("input", () => {
    requiredData = allData.filter((dish) => {
      let array = dish.mealType;
    ...
    document.getElementById("box").innerHTML = ``;
    renderAllCards(requiredData);
  });
}
letterSearch();

// Filter the cards based on the rating
function filterCards(e) {
  let requiredrating = Number(e.target.value);
  let filterData;
  if (requiredrating == 4.5) {
...
// Function to return difficult based cards
function showData(e) {
  let newData = allData.filter((obj) => obj.difficulty == e);
  document.getElementById("box").innerHTML = "";
  renderAllCards(newData);
}
...
document.getElementById("previousPage").addEventListener("click", () => {
  newData = allData.slice(count - 20, count - 10);
  document.getElementById("box").innerHTML = "";
  renderAllCards(newData);
  count -= 10;
  checkPrevNext();
